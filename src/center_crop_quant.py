...
def run_unified_metadata_and_quantification(
    ...
    print(f"Found {len(target_ref_folders)} datasets groups blocks. Booting processing loop...")
    
    for ref_folder_path in target_ref_folders:
        print(f"Working on reference folder: {ref_folder_path}")
        ...
        # Instantiate output storage arrays directories
        ...
        # Extract UI layout information once per subfolder directory block matrix
        ...
        # Gather file items containing correction indices tokens
        ...
        for file_path in path_bf_files:
            ...
            print(f" Processing Matrix Entry: {filename_bf}")
            
            img_raw_bf = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
            
            if img_raw_bf is None: continue
            img_h, img_w = img_raw_bf.shape[:2]
            
            # PHASE 1: Parse Metadata Structural Strings
            ...
            # PHASE 2: Image Analysis Outline Segmentations
            ...
            # Dynamic sibling tracks conversions
            ch2_dir_path = file_path.replace(outline_folder, quant_folder)
            sibling_ch2_path = os.path.join(os.path.dirname(ch2_dir_path), os.path.basename(ch2_dir_path).replace(outline_file_token, quant_file_token))
            
            ch4_dir_path = file_path.replace(outline_folder, nuc_folder)
            sibling_ch4_path = os.path.join(os.path.dirname(ch4_dir_path), os.path.basename(ch4_dir_path).replace(outline_file_token, nuc_file_token))
            ...
            # PHASE 3: Sibling Channel Slicing Crops & Calculations
            if os.path.exists(sibling_ch2_path):
                ...
                print(f"  [Warning Path Error] Sibling channel 2 missing: {sibling_ch2_path}")
            ...
            if os.path.exists(sibling_ch4_path):
                ...
                # Hand over the loaded cellpose_model here
                ...
                print(f"  [Warning Path Error] Sibling channel 4 missing: {sibling_ch4_path}")
            ...
            record = {
                ...
                "Filename": filename_bf,
                "Shape": shape,
                "Position in 6wp": position,
                "Treatment": treatment,
                "Cell-Line": cell_line,
                "Live or Timepoint": live_time,
                "Colony_Area_px": colony_area,
                "Colony_Perimeter_px": perimeter,
                "Colony_Aspect_Ratio": aspect_ratio,
                "Colony_Roundness": roundness,
                "Colony_Solidity": solidity,
                f"{quant_label}_Thresh_Area_In_Mask_px": ch2_area,
                f"{nuc_label}_Count": total_nuclei,
                f"Total_{nuc_label}_Area_In_Mask_px": dapi_area,
                "Overall_Density_nuclei_per_1000px2": overall_density,
                f"Pct_{quant_label}_per_{mask_label}_Area": percent_ch2_in_colony,
                f"Pct_{quant_label}_per_{nuc_label}_Area": percent_ch2_per_dapi,
                "Mean_Radial_Distance_um": mean_rad,
                "Median_Radial_Distance_um": median_rad,
                "Stdev_Radial_Distance_um": stdev_rad
            }
            record.update(channel_ui_info)
            all_records.append(record)

    # ================================================================================
    # EXPORT MASTER EXCEL DATABASE & PARALLEL RAW DISTRIBUTION SPREADSHEETS
    # ================================================================================
    if not all_records:
